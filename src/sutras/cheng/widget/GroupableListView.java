 ...
 * @author chengkai
 */
public class GroupableListView extends ExpandableListView implements
	...
	public GroupableListView(Context context, AttributeSet attrs, int defStyle) {
		super(context, attrs, defStyle);
		registerListener();
	}

	public GroupableListView(Context context, AttributeSet attrs) {
		super(context, attrs);
		registerListener();
	}

	public GroupableListView(Context context) {
		super(context);
		registerListener();
	}
	...
	public void setHeaderView(View view, ImageView image, TextView text) {
		setHeaderView(view);
		this.headerImage = image;
		this.headerText = text;
	}

	public void setHeaderView(int layout, int resImage, int resText) {
		View view = LayoutInflater.from(getContext()).inflate(layout, null);
		setHeaderView(view);
		this.headerImage = (ImageView) view.findViewById(resImage);
		this.headerText = (TextView) view.findViewById(resText);
	}
	...
	protected void onLayout(boolean changed, int left, int top, int right,
		...
		int state = mAdapter.getGroupHeaderState(groupPos, childPos);
	...
	public void configureHeaderView(int groupPosition, int childPosition) {
		...
		int state = mAdapter.getGroupHeaderState(groupPosition, childPosition);
		switch (state) {
		case GroupHeaderAdapter.PINNED_HEADER_GONE: {
			mHeaderViewVisible = false;
			break;
		}
		case GroupHeaderAdapter.PINNED_HEADER_VISIBLE: {
			mAdapter.configureGroupHeader(mHeaderView, headerText,
		...
		case GroupHeaderAdapter.PINNED_HEADER_PUSHED_UP: {
			...
			mAdapter.configureGroupHeader(mHeaderView, headerText,
					groupPosition, childPosition, alpha);
	...
	public interface GroupHeaderAdapter {
		 ...
		 *         其中之一
		 */
		int getGroupHeaderState(int groupPosition, int childPosition);

		/**
		 * 配置分组Header, 让分组Header 知道显示的内容
		 * 
		 * @param header
		 * @param groupPosition
		 * @param childPosition
		 * @param alpha
		 */
		void configureGroupHeader(View header, TextView headerText,
